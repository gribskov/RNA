package Topology;
#-----------------------------------------------------------------------------
# $Id: Topology.pm,v 2.0 2014/07/08 15:14:34 gribskov Exp $
    ...
    foreach my $char ( @varray ) {
        ...
        if ( $type ) {                  # skip spaces (type=0)
            ...
            if ( $value{$char} > 0 ) {              # left parentheses have value>0
                ...
                $right = $pos;
                $left = pop @{$stack[$type]};
                last unless defined $left;          # this should only happen if parentheses are unbalanced, possibly because of un-annotated pseudoknot
                if ( $right - $old_right > $max_unpaired or
                     $old_left - $left > $max_unpaired or $type != $old_type ) {
                    unless ( $first_left < 0 ) {
                        $nstem++;
                        my $leftv = substr( $vienna, $old_left-1, $first_left-$old_left+1 );
                        my $rightv = substr( $vienna, $first_right-1, $old_right-$first_right+1 );
                        #print "stem $nstem: $left,$old_left,$first_left,$leftv to $right,$first_right,$old_right,$rightv\n";
                        my $stem = Stem->new( [$old_left,$first_left,$first_right, $old_right, $leftv, $rightv] ); 
                        $self->addStem($stem);
                    }
                    $first_left = $left;
                    $first_right = $right;
...
sub mfoldMultiple{
	...
	die "Topology::mfoldMultiple, unable to open input CT file ($filename)\n\n";
...
#            push @stemlist, basepairToStem( @pair );
	    #-------
	    my @local_stem = &basepairToStem( @pair );
	    # split the stems one strucuture at a time
	    # this should be done before the uniqueness test for stem... 
	    foreach my $lstem (@local_stem) {
		push @stemlist, splitStems( $max_unpaired, $lstem );
	    }  
	    #----  
            my ( $length, $dg, $equal, $energy, $id, $doc ) = split " ", $line, 6;
    ...
    $self->sequence( $seq );    # sequence for all structures should be the same
	...
	foreach my $stem ( @stemlist ) {
	    ...
	    $key .= $stem->vienna_right.$stem->vienna_left;
	    $unique{$key} = $stem;
	}
	my @ustem;
	foreach my $stem ( keys %unique ) {
#        push @ustem, splitStems( $max_unpaired, $unique{$stem} ); 
	    push @ustem, $unique{$stem};
	}
...
# parseClusterFile
#
# This subroutine can be use to parse cluster files generated from sfold.
# Only base pairs that satisfy the threahold are considered. The input file are the ones with .2dhist extension
# 
# USAGE:
#    my $nstem = $Topology_obj->parseClusterFile($file, $threshold);
#--------------------------------------------------
	
sub  parseClusterFile {
    my ($self, $cluster_file, $threshold) = @_;
    my $max_unpaired = 2; #3 is the preferred value
    
    open( my $CLUSTER, "<", $cluster_file )  
	or die "Cannot read file: $cluster_file\n";
    ...
    push @stemlist, basepairToStem( @pair );
    #split stems first, then go into unique stems idea...
    # create unique stemlist
#---------------------------
#    foreach my $stem (sort {$a->{'left1'}<=>$b->{'left1'}} @stemlist) {
#	print "[ $stem->{left1}, $stem->{left2}, $stem->{right1}, $stem->{right2}]\n";
#	#push @ustem, splitStems( $max_unpaired, $unique{$stem} );
#    }
#    print "\nPrinting before unique stem done...\n";
#---------------------
...
# and 6, stem 2 pairing postions 2 and 4, and stem 3 pairing positions 3 and 6.
#
# USAGE
...
#         }
    ...
    my $pos = 0;
    while ( $pos < @$parray ) {
        my $stem = Stem->new(   { left1        => $$parray[$pos],
                                  left2        => $$parray[$pos],
                                  right1       => $$parray[$pos+1],
                                  right2       => $$parray[$pos+1],
                                  vienna_left  => '(',
...
sub energyplot {
    ...
    my @sorted_stems = sort { $a->[3] <=> $b->[3] || $a->[0] <=> $b->[0] || 
                              $a->[1] <=> $b->[1] } @stems;  
        ...
        if (${$newstem}[3] == $best_e) {
            ...
            for my $uniqstem (@uniq_stems) {
                if ( $ls_new <= ${$uniqstem}[1] && $le_new >= ${$uniqstem}[0]) {   
                    ...
                    if ( $rs_new <= ${$uniqstem}[3] && $re_new >= ${$uniqstem}[2]) {   
...
sub rnaml{
    ...
    open ( RNAML, "<$filename" ) || die "Unable to open RNAML file $filename\n\n";
    ...
    while ( my $line = <RNAML> ) {
        ...
        if ( $line =~ /<\s*data/ ) { if ( $in_sequence ) { $in_data = 1; next;} }
        ...
        # structure elements, e.g.,  <base-pair _5p-base-num="2"  _3p-base-num="403"/>
            ...
            my ( $l, $r ) = $line =~ /base-pair\s*_5p-base-num\s*=\s*\"(\d+)\"\s*_3p-base-num\s*=\s*\"(\d+)\"/;
            unless ( defined($l) && defined($r) ) { print "undefined l or r:$line\n";}
...
#	print "$l, $r\n";
    ...
    while ( my $pair = shift @pair ) {
        ...
        my $need_new_stem = 0;
        my $left_vienna = "";
        #print "left:$left   new left:$left_new\n";
        for ( my $l=$left+1; $l<$left_new; $l++ ) {
            $left_vienna .= ".";
            #print "   l:$l   index[$l]:$index[$l]\n";
            if ( defined( $index[$l] ) ) {
                #print "      NEED NEW!\n";
                $need_new_stem = 1;
                last;
        ...
        my $right_vienna = "";
#        print "right:$right   new right:$right_new\n";
        my $r0 = $right;
        my $r1 = $right_new;
        if ( $right_new > $right ) {
            $r0 = $right_new;
        ...
        if ( $need_new_stem ) {
            ...
            #$stem->dump;
            $stem = new Stem;
            #print "new stem: [$left_new, $left_new, $right_new, $right_new]\n";
            $stem->setStem( [$left_new, $left_new, $right_new, $right_new, "(", ")"] );
            $n_stem++;
...
# unpaired bases.  If yes, break it into two stems. Split stem can also detect bulges on 
# either half stems, this prevents stem splitting at bulges.
#
#            (((...)...)) there is a bulge on right half stem, do not split; 
#            ((.(...)...)) split in this case
#
# USAGE
...
#    print "Target; $target\n";
	...
	#----------------------------------------------------------------------------
	#If a stem has both bulges and unparied base, the break might happen at bulges 
	#if the bulge size if bigger than the number of unpaired bases...
	#we need a test to capture the bulge cases...
	#this case captures bulge on one side and no unpaired bases on the other half ... DONE!
	#-----------------------------------------------------------
...
#        print "\nsplitting $left     $right\n";
	my ($ll, $lc, $lr)=('','','');
	while ($left =~ /\.{$max_unpaired,}/g) {
	    $ll = $`;
	    $lc = $&;
	    $lr = $'; #'
	    
	    #test for bulge in the left half stem
	    my $num_par_left = $ll =~ tr/()/()/;
	    my $num_par_right = $lr =~ tr/()/()/;
	    #now take the right half stem, to find the corresponding parts
	    my ($right_l2) = $right =~ /((\.*[()]){$num_par_right})/;
	    my ($right_r2) = $right =~ /(([()]\.*){$num_par_left})$/;
#	    print "search for bulge in left half stem...    lp1: $ll;   lp2: $lr;            rp2: $right_l2;   rp1: $right_r2\n";
	    if (length($right) == length($right_l2) + length($right_r2)) { #clean bulge on left half
#		print "found bulge on left half stem\n";
		($ll, $lc, $lr)=('','','');
	    } else {
		last;
	    }
	}
        my $leftdots = length( $lc );
	
	my ($rl, $rc, $rr)=('','','');
        while ($right =~ /\.{$max_unpaired,}/g) {
	    $rl = $`;
	    $rc = $&;
	    $rr = $'; #'

	    #test for bulge in the right half stem
	    my $num_par_left = $rl =~ tr/()/()/;
	    my $num_par_right = $rr =~ tr/()/()/;
	    #now take the right half stem, to find the corresponding parts
	    my ($left_l2) = $left =~ /((\.*[()]){$num_par_right})/;
	    my ($left_r2) = $left =~ /(([()]\.*){$num_par_left})$/;
#	    print "search for bulge in right half stem...    lp1: $left_l2;   lp2: $left_r2;            rp2: $rl;   rp1: $rr\n";
	    if (length($left) == length($left_l2) + length($left_r2)) { #clean bulge on left half
#		print "found bulge on right half stem\n";
		($rl, $rc, $rr)=('','','');
	    } else {
		last;
	    }
	}
        my $rightdots = length( $rc );
   	
	#if both leftdots and rightdots turn out to be zero because of bulges, then no need to split the stem.... 
	if ($rightdots == 0 && $leftdots == 0) {
#	    print "stem added to list: $stem\n";
	    push @new_stems, $stem;
            next;
	}
...
#	print "rightdots: $rightdots > leftdots: $leftdots\n";
	...
	my ($nparenl, $newl1, $newr1, $nparenr, $newl2,  $newr2);
	if ( $rightdots > $leftdots ) {
#	    print "||| rightdots > leftdots\n";
	    #right is getting split
	    $nparenl = $template_left =~ tr/()/()/;
#	    print " left template:$template_left   n:$nparenl\n";
	    $nparenr = $template_right =~ tr/()/()/;
#	    print "right template:$template_right   n:$nparenr\n";
	    #all the values are applicable to the right half of the stem
	    
	    ( $newl1 )= $left =~ /((\.*[()]){$nparenr})/;
#	    print "      new left:$newl1    from real $left\n";
	    ( $newr1 )= $right =~ /(([()]\.*){$nparenr})$/;
#	    print "     new right:$newr1    from real $right\n";
...
sub XIOSWrite{
	...
	if ( defined $filename ) {
	    ...
	    open( my $out, '>', $filename ) or 
	       die "Topology::XIOSWrite unable to open $filename for output\n\n";
        print $out $str;
	}
...
#
# optionally, $filename can be a string containing an xios formatted string
#
# USAGE
...
sub XIOSRead{
	...
	# if filename is readable open as file, otherwise read as string
	unless ( $filename =~ /<XIOS/ ) {
	    open( my $in, "<", $filename ) or  
            die "Topology::XIOSRead, unable to open XIOS file ($filename)\n\n";
	} 
    ...
    $topology->sequenceId( $xios->{information}->{sequence}->{id} );
...
sub informationFormat {
    ...
    if ( $seq || $seqid || $seqdoc ) { 
        ...
        if ( $ss ) {
            $ss =~ s/ //g;  # remove spaces
            $str .= qq{      <secondary>$ss</secondary>\n};
        }
        $str .= qq{    </sequence>\n};
    }
...
sub stemlistFormat{
    ...
    foreach my $stem ( @{$self->stemList} ) {
        ...
        if ( $size[0] < length $stem->{left1} )        { $size[0] = length $stem->{left1} }
        if ( $size[1] < length $stem->{left2} )        { $size[1] = length $stem->{left2} }
        if ( $size[2] < length $stem->{right1} )       { $size[2] = length $stem->{right1} }
        if ( $size[3] < length $stem->{right2} )       { $size[3] = length $stem->{right2} }
        if ( $size[4] < length $stem->{vienna_left} )  { $size[4] = length $stem->{vienna_left} }
        if ( $size[5] < length $stem->{vienna_right} ) { $size[5] = length $stem->{vienna_right} }
    }
    ...
    my $fmt = "    \%$size[6]d \%$dsize.1f [ \%$size[0]d \%$size[1]d   \%$size[2]d \%$size[3]d ]  \%$size[4]s   \%-$size[5]s\n";
    # print "format:$fmt\n";
...
sub stemlistRead{
	...
	foreach my $line ( split "\n", $text ) {
	    ...
	    unless ( defined $$stemin{vienna_right} ) { $$stemin{vienna_right} = "" }
...
sub stemListAsArray{
	...
	foreach my $stem ( @{$topology->stemList} ) {
	    push @{$stemarray}, [ $stem->left1,  $stem->left2, 
	                          $stem->right1, $stem->right2 ];
	}
...
sub adjacencyMatrix{
	...
	foreach my $i ( 0 .. $#stem ) {
	    my $s = $stem[$i];
	    my ( $a1, $a2, $a3, $a4 ) = ( $s->left1, $s->left2, $s->right1, $s->right2 );
	    my @row;
	    foreach my $j ( 0 .. $#stem ) {
	        if ( $i == $j ) {
	            push @row, '-';
		...
		my $s = $stem[$j];
		my ( $b1, $b2, $b3, $b4 ) = ( $s->left1, $s->left2, $s->right1, $s->right2 );
...
sub overlapType{
	...
	my $sa = $$stemlist[$n_a];
	my $sb = $$stemlist[$n_b];
	my @coord = ( $sa->left1, $sa->left2, $sa->right1, $sa->right2,
...
sub overlapToCanonical{
    ...
    @coor = ( $sa->left1, $sa->left2, $sa->right1, $sa->right2,
              $sb->left1, $sb->left2, $sb->right1, $sb->right2 );
...
#-----------------------------------------------------------------------------
# $Log: Topology.pm,v $
# Revision 2.0  2014/07/08 15:14:34  gribskov
# Updated vs mrg121003 branch, renamed current versions to be 2.0
#
# Revision 1.62  2014/07/08 13:20:13  gribskov
# Merged v 1.61 with 1.59.2.18 most if the differences seem minor or were
# in comments.  Reviewed conflicts carefully but did not test
#
# Revision 1.61  2012/10/31 05:16:55  rahmanr
# no stems are split, after the uniqueness test. Fixed bug.
#
# Revision 1.60  2012/10/18 18:28:39  rahmanr
# Used Michael's 1.59.2.3 as a starting point to make a few changes to splitStems:
# Added test for bulge, so that stems are not split at bulges
#              (((...)...)) there is a bulge on right half stem, do not split;
#              ((.(...)...)) split in this case
# fixed the case where right half stem had more dots than left half stem
# fixed scoping error for variables
# working version of a parser for clusterfile, but it needs more work
#
# Revision 1.59.2.18  2014/01/22 18:13:37  gribskov
# Added storage for secondary structure string.
#
# Revision 1.59.2.17  2013/06/24 18:34:29  gribskov
# Improved XIOSRead to read a file or a string containing a XIOS formatted structure.
#
